...
int if_sdio_card_to_host(struct if_sdio_card *card)
{
    ...
    RDA5890_DBGLAP(RDA5890_DA_SDIO, RDA5890_DL_NORM, "if_sdio_card_to_host, size = %d, aligned size = %d\n", size, chunk);
    ...
    RDA5890_DBGLAP(RDA5890_DA_SDIO, RDA5890_DL_VERB, "if_sdio_card_to_host, read done\n");
...
out:
    RDA5890_DBGLAP(RDA5890_DA_SDIO, RDA5890_DL_DEBUG, "if_sdio_card_to_host >>>\n");
    return ret;
}
...
void if_sdio_sleep_worker(struct work_struct *work)
{
    ...
    card = container_of(work, struct if_sdio_card, sleep_work.work);
...
#define SDIO_HOST_WRITE_BATCH_SIZE   512
    ...
    while (1)
    {
        ...
        RDA5890_DBGLAP(RDA5890_DA_SDIO, RDA5890_DL_DEBUG, "if_sdio_host_to_card_worker, send one packet, size = %d\n", packet->nb);
        /* write length */
        size = packet->nb/4;
        size_l = (u8)(size & 0xff);
        ...
        while(bytes_left)
        {
            ...
            sdio_claim_host(card->func);
            ret = sdio_writesb(card->func, IF_SDIO_FUN1_FIFO_WR, packet->buffer + offset, batch);
            sdio_release_host(card->func);
            if (ret) {
            ...
            RDA5890_DBGLAP(RDA5890_DA_SDIO, RDA5890_DL_DEBUG, "write batch %d, offset = %d\n", batch, offset);

            offset += batch;
            bytes_left -= batch;
        }

        RDA5890_DBGLAP(RDA5890_DA_SDIO, RDA5890_DL_DEBUG, "if_sdio_host_to_card_worker, send one packet done\n");
release:
        kfree(packet);
        packet = NULL;
    }
...
static int if_sdio_host_to_card(struct rda5890_private *priv, u8 *buf, u16 nb, unsigned char packet_type)
{
    int ret = 0;
    ...
    RDA5890_DBGLAP(RDA5890_DA_SDIO, RDA5890_DL_NORM, "if_sdio_host_to_card, size = %d, aligned size = %d\n", nb, size);
...
static void if_sdio_interrupt(struct sdio_func *func)
{
    ...
    RDA5890_DBGLAP(RDA5890_DA_SDIO, RDA5890_DL_VERB, "if_sdio_interrupt, status = 0x%02x\n", status);
    ...
    if (status & IF_SDIO_INT_ERROR)
    {
        ...
        RDA5890_DBGLAP(RDA5890_DA_SDIO, RDA5890_DL_TRACE, "%s, INT_ERROR\n", __func__);
    }

out:
    RDA5890_DBGLAP(RDA5890_DA_SDIO, RDA5890_DL_DEBUG, "%s <<< ret=%d \n", __func__, ret);
    return ret;
}
...
static int if_sdio_probe(struct sdio_func *func, const struct sdio_device_id *id)
{
    ...
    if (sdio_test_flag) {
        ...
        RDA5890_DBGLAP(RDA5890_DA_SDIO, RDA5890_DL_TRACE, "SDIO TESTING MODE\n");
